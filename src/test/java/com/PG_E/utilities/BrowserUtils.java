...
public class BrowserUtils{
    ...
    public static void sendKeysWithTimeOut (WebElement element, String str, int timeout) {
        WebDriverWait wait = new WebDriverWait(Driver.getDriver(), timeout);
    ...
    public static void clickUntilElementVisible(WebElement elementToClick, WebElement elementToSee) {
        for (int i =0; i<5;i++) {
            clickWithTimeOut(elementToClick, 10);
            try {
                if (elementToSee.isDisplayed()) {
                    break;
                } else {
                    continue;
                }
